        ...
        <div className="flex gap-4 transition-all duration-300">
          {currentCards.map((project) => (
            <div
              ...
              className="min-w-[250px] bg-white rounded-xl shadow-md overflow-hidden hover:scale-105 transition-transform cursor-pointer"
            >
              <img src={project.images[0]} alt={project.title} className="w-full h-40 object-cover" />
              <div className="p-4">
                <h3 className="font-bold text-lg">{project.title}</h3>
                <p className="text-sm text-gray-600">{project.summary.substring(0, 60)}...</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="flex justify-center gap-2">
        {Array.from({ length: totalPages }).map((_, i) => (
          <button
            key={i}
            onClick={() => setPage(i)}
            className={`w-3 h-3 rounded-full ${i === page ? 'bg-gray-800' : 'bg-gray-400'}`}
          />
        ))}
      </div>
    </div>
  ...
  return (
    <section id="projects" className="py-12 bg-white">
      <div className="text-center max-w-7xl mx-auto">
        <h2 className="text-4xl sm:text-5xl font-extrabold mb-12 text-center text-gray-900" style={{ fontFamily: "Anton, sans-serif" }}>
          Projects
        </h2>
      </div>
          ...
          >
            ...
            <div className="relative">
              ...
              <img
                ...
                className="w-full object-cover rounded-t-lg max-h-[400px] mx-auto"
              />
              <button
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 p-6 text-white">
              ...
              <div>
                ...
                <p className="whitespace-pre-line mb-4">{selectedProject.summary}</p>
              </div>
            </div>
            ...
            <div className="p-6">
              ...
              <div className="grid grid-cols-4 gap-4">
                {selectedProject.images.map((image, index) => {
                  const isLargeImage =
                    index === 0 || index === selectedProject.images.length - 1;
                  return (
                    <div
                      key={index}
